...
module IpAddressLookupTwoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpAddressLookupTwoResponse.new({})
    ...
    if fetch_err
      ...
      response = IpAddressLookupTwoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpAddressLookupTwoResponse.new(fetched)
    else
