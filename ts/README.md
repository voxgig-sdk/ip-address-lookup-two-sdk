...
`client.Ipn()` — each with a small set of operations (`load`)
...
```ts
import { IpAddressLookupTwoSDK } from '@voxgig-sdk/ip-address-lookup-two-sdk'
...
| `tester(testopts?, sdkopts?)` | `IpAddressLookupTwoSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `IpAddressLookupTwoSDK.test(testopts?, sdkopts?)` | `IpAddressLookupTwoSDK` | Create a test-mode client. |
...
| `client` | `client(): IpAddressLookupTwoSDK` | Return the parent SDK client. |
...
├── src/
│   ├── IpAddressLookupTwoSDK.ts        # Main SDK class
