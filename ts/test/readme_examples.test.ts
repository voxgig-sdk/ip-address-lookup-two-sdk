...
//     (\`new IpAddressLookupTwoSDK(...)\` / \`IpAddressLookupTwoSDK.test(...)\` become
//     \`IpAddressLookupTwoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpAddressLookupTwoSDK'
...
const TEST_SEED = {"entity":{"ipn":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpAddressLookupTwoSDK(...)\` and any
// \`IpAddressLookupTwoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpAddressLookupTwoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpAddressLookupTwoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
