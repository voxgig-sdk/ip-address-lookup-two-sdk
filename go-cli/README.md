# ip-address-lookup-two-cli

boru-driven command-line client **and** interactive REPL for the IpAddressLookupTwo
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-address-lookup-two-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-address-lookup-two-cli --help

# 3. Provide credentials once, via the environment
export IP_ADDRESS_LOOKUP_TWO_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-address-lookup-two-cli load 1 ipn            # {id:1} shorthand
./ip-address-lookup-two-cli load '{id:1}' ipn       # explicit match map

# 5. Override the API base URL for a single call
IP_ADDRESS_LOOKUP_TWO_BASE=https://api.example.com ./ip-address-lookup-two-cli load 1 ipn

# 6. No arguments -> interactive REPL
./ip-address-lookup-two-cli
ip-address-lookup-two> load 1 ipn
ip-address-lookup-two> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-address-lookup-two-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-address-lookup-two-cli load 1 ipn
   ```
...
```sh
./ip-address-lookup-two-cli load 1 ipn          # scalar shorthand for {id:1}
./ip-address-lookup-two-cli load '{id:1}' ipn     # explicit match map
```
...
./ip-address-lookup-two-cli load 1 ipn
```
...
Run with no arguments to open a REPL (prompt `ip-address-lookup-two>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-address-lookup-two-cli
ip-address-lookup-two> load 1 ipn
ip-address-lookup-two> /help
ip-address-lookup-two> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-address-lookup-two-cli`. |
...
(`./ip-address-lookup-two-cli <expr>`) and interactively (the REPL), and expressions compose the
