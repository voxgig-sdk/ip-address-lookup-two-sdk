// Command IpAddressLookupTwo-cli is a CLI + REPL for the IpAddressLookupTwo SDK,
...
const prompt = "ip-address-lookup-two"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "ipn"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IP_ADDRESS_LOOKUP_TWO_APIKEY (api key), IP_ADDRESS_LOOKUP_TWO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IP_ADDRESS_LOOKUP_TWO_APIKEY carries the API key and
	// IP_ADDRESS_LOOKUP_TWO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_ADDRESS_LOOKUP_TWO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_ADDRESS_LOOKUP_TWO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpAddressLookupTwoSDK(opts)
