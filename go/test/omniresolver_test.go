...
import (
	...
	omni "github.com/voxgig-sdk/ip-address-lookup-two-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpAddressLookupTwoError); is {
				out := map[string]any{
					"name":    "IpAddressLookupTwoError",
...
func omniCtx(arg any, client *sdk.IpAddressLookupTwoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpAddressLookupTwoSDK); is {
			client = live
