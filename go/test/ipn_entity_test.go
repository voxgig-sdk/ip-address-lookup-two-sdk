...
func TestIpnEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func ipnBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID"])
	...
	if env["IP_ADDRESS_LOOKUP_TWO_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewIpAddressLookupTwoSDK(core.ToMapAny(mergedOpts))
	}
