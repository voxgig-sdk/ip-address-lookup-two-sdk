...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IPADDRESSLOOKUPTWO_TEST_LIVE") == "TRUE" ||
		os.Getenv("IPADDRESSLOOKUPTWO_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IPADDRESSLOOKUPTWO_TEST_EXPLAIN"); explain != "" {
