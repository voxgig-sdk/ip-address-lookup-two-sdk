...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IP_ADDRESS_LOOKUP_TWO_TEST_LIVE") == "TRUE" ||
		os.Getenv("IP_ADDRESS_LOOKUP_TWO_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IP_ADDRESS_LOOKUP_TWO_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.IpAddressLookupTwoSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.IpAddressLookupTwoSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.IpAddressLookupTwoError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.IpAddressLookupTwoSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.IpAddressLookupTwoError{Msg: msg, Code: code}
}
