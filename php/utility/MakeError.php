...
class IpAddressLookupTwoMakeError
{
    public static function call(?IpAddressLookupTwoContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpAddressLookupTwoContext([], null);
        }
        $op = $ctx->op ?? new IpAddressLookupTwoOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpAddressLookupTwoResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpAddressLookupTwoError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IpAddressLookupTwoError('', $msg, $ctx);
        ...
        if ($err instanceof IpAddressLookupTwoError) {
