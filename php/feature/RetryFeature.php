...
// IpAddressLookupTwo SDK retry feature
...
class IpAddressLookupTwoRetryFeature extends IpAddressLookupTwoBaseFeature
{
    ...
    public function init(IpAddressLookupTwoContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpAddressLookupTwoContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpAddressLookupTwoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpAddressLookupTwoContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
