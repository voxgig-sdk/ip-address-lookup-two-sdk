...
class IpnEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function ipn_basic_setup($extra)
{
    ...
    $client = IpAddressLookupTwoSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["IP_ADDRESS_LOOKUP_TWO_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new IpAddressLookupTwoSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["IP_ADDRESS_LOOKUP_TWO_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
