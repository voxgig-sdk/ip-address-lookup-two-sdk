...
//    only - a typed IpAddressLookupTwoContext there would make every ctx
...
final class IpAddressLookupTwoOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpAddressLookupTwoContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new IpAddressLookupTwoSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new IpAddressLookupTwoResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new IpAddressLookupTwoError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new IpAddressLookupTwoResponse($clean['response']);
    ...
    public static function err_from_map($m): ?IpAddressLookupTwoError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpAddressLookupTwoError(is_string($code) ? $code : '', $msg);
    }
