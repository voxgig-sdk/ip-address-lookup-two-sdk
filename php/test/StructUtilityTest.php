...
// IpAddressLookupTwo SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IpAddressLookupTwoOmni::makeStructRunner(
    ...
    {
        return IpAddressLookupTwoOmni::objmodel(IpAddressLookupTwoOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return IpAddressLookupTwoOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return IpAddressLookupTwoOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IpAddressLookupTwoOmni::structfix($now, Struct::undef()))
                        !== json_encode(IpAddressLookupTwoOmni::structfix($val, Struct::undef()))) {
