# ip-address-lookup-two-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IpAddressLookupTwo SDK as
two agent tools — `ip-address-lookup-two_list` and `ip-address-lookup-two_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-address-lookup-two-mcp)
make build

# 2. Provide credentials via the environment
export IP_ADDRESS_LOOKUP_TWO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-address-lookup-two \
  -- /absolute/path/to/ip-address-lookup-two-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-address-lookup-two-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-address-lookup-two_list: first page of records
{ "entity": "ipn" }
{ "entity": "ipn", "query": { } }

// ip-address-lookup-two_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-address-lookup-two-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-address-lookup-two \
     -- "$PWD"/dist/*/ip-address-lookup-two-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-address-lookup-two_list` and `ip-address-lookup-two_load` tools now appear
   in new sessions. Ask the agent to *"list ipn using ip-address-lookup-two"*
   and it calls `ip-address-lookup-two_list` with `{"entity":"ipn"}`.
...
```sh
export IP_ADDRESS_LOOKUP_TWO_APIKEY=sk_live_xxx            # API key
export IP_ADDRESS_LOOKUP_TWO_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-address-lookup-two_list` tool
...
### Call the `ip-address-lookup-two_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-address-lookup-two_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-address-lookup-two_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_ADDRESS_LOOKUP_TWO_APIKEY` | API key sent with every request. |
| `IP_ADDRESS_LOOKUP_TWO_BASE` | Optional override of the API base URL. |
...
```sh
./ip-address-lookup-two-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-address-lookup-two_load","arguments":{"entity":"ipn","query":{"id":1}}}}'
```
