# IpAddressLookupTwo Python SDK

The Python SDK for the IpAddressLookupTwo API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Ipn(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.IpAddressLookupTwo(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── ipaddresslookuptwo_sdk.py         -- Main SDK module
