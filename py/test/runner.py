# IpAddressLookupTwo SDK test runner
...
class IpAddressLookupTwoTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpAddressLookupTwoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpAddressLookupTwoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpAddressLookupTwoTestRunner.getenv("IP_ADDRESS_LOOKUP_TWO_TEST_LIVE")
        override = IpAddressLookupTwoTestRunner.getenv("IP_ADDRESS_LOOKUP_TWO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpAddressLookupTwoTestRunner.getenv(key)
        ...
        explain = IpAddressLookupTwoTestRunner.getenv("IP_ADDRESS_LOOKUP_TWO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IP_ADDRESS_LOOKUP_TWO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpAddressLookupTwoTestRunner._test_control is not None:
            return IpAddressLookupTwoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpAddressLookupTwoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpAddressLookupTwoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpAddressLookupTwoTestRunner.load_test_control()
...
def load_env_local():
    IpAddressLookupTwoTestRunner.load_env_local()


def env_override(m):
    return IpAddressLookupTwoTestRunner.env_override(m)


def entity_data(v):
    return IpAddressLookupTwoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpAddressLookupTwoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpAddressLookupTwoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpAddressLookupTwoTestRunner.load_test_control()


def live_client_options():
    return IpAddressLookupTwoTestRunner.live_client_options()


def live_delay_ms():
    return IpAddressLookupTwoTestRunner.live_delay_ms()
