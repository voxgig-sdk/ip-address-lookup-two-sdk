...
class IpnEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def ipn_basic_setup(extra)
  ...
  entid_env_raw = ENV["IP_ADDRESS_LOOKUP_TWO_TEST_IPN_ENTID"]
  ...
  if env["IP_ADDRESS_LOOKUP_TWO_TEST_LIVE"] == "TRUE"
    ...
    client = IpAddressLookupTwoSDK.new(Helpers.to_map(merged_opts))
  end
