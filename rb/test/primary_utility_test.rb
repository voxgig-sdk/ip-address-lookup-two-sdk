...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = IpAddressLookupTwoOmni.make_runner(TEST_JSON_FILE, IpAddressLookupTwoSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    IpAddressLookupTwoError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : IpAddressLookupTwoOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = IpAddressLookupTwoOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
