# IpAddressLookupTwo SDK Ipn entity
...
class IpnEntity
  ...
  def data_set(args)
    if args
      @_data = IpAddressLookupTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = IpAddressLookupTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Ipn, Hash] the loaded Ipn; raises IpAddressLookupTwoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpAddressLookupTwoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
